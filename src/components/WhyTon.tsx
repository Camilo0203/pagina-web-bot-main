        ...
        <div className="grid items-center gap-16 lg:grid-cols-2 lg:gap-20">
          <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>
            <div className="mb-8 flex items-center gap-4">
              ...
              <span className="text-[10px] font-bold uppercase tracking-[0.4em] text-indigo-400">{t('why.tag')}</span>
            </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              {highlights.map((item) => (
                <div key={item.label} className="cinematic-glass rounded-2xl p-6">
                  <div className="mb-2 text-[11px] font-bold uppercase tracking-[0.25em] text-indigo-300">{item.label}</div>
                  <div className="mb-2 text-3xl font-bold text-white">{item.value}</div>
                  <div className="text-[10px] font-bold uppercase tracking-[0.22em] text-slate-500">{item.sub}</div>
                </div>
              ))}
            </div>
          </motion.div>
